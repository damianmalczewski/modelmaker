...
/**
 * {@code jakarta.validation} annotation configuration: whether the feature is on, and where its
 * annotations sit (fields, getters, or both). Built through {@link #builder()}; read back through
 * {@code ModelOptions}.
 */
public final class ValidationConfig {

  private static final ValidationConfig DEFAULT_VALUE = new ValidationConfig(false, false, true);

  private final boolean enabled;
  private final boolean annotateFields;
  private final boolean annotateGetters;

  private ValidationConfig(boolean enabled, boolean annotateFields, boolean annotateGetters) {
  ...
  /**
   * Applies a per-schema {@link ValidationOverride} over this config; unset override flags keep
   * this instance's value.
   *
   * @param override the override.
   * @return the resolved config.
   */
  ValidationConfig withOverride(ValidationOverride override) {
  ...
  public boolean equals(@Nullable Object obj) {
    ...
    if (!(obj instanceof ValidationConfig other)) {
      return false;
    }
  ...
  /** Accumulates a {@link ValidationConfig}; starts at the feature defaults. */
  public static final class Builder {
     ...
     *
     * @return a new {@link ValidationConfig}.
     */
    public ValidationConfig build() {
      return new ValidationConfig(enabled, annotateFields, annotateGetters);
    }
  }
}
