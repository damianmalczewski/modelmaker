...
/**
 * OpenAPI {@code @Schema} annotation configuration: whether the feature is on, and where its
 * annotations sit (fields, getters, or both). Built through {@link #builder()}; read back through
 * {@code ModelOptions}.
 */
public final class OpenApiConfig {

  private static final OpenApiConfig DEFAULT_VALUE = new OpenApiConfig(false, false, true);

  private final boolean enabled;
  private final boolean annotateFields;
  private final boolean annotateGetters;

  private OpenApiConfig(boolean enabled, boolean annotateFields, boolean annotateGetters) {
  ...
  /**
   * Applies a per-schema {@link OpenApiOverride} over this config; unset override flags keep this
   * instance's value.
   *
   * @param override the override.
   * @return the resolved config.
   */
  OpenApiConfig withOverride(OpenApiOverride override) {
  ...
  public boolean equals(@Nullable Object obj) {
    ...
    if (!(obj instanceof OpenApiConfig other)) {
      return false;
    }
  ...
  /** Accumulates a {@link OpenApiConfig}; starts at the feature defaults. */
  public static final class Builder {
     ...
     *
     * @return a new {@link OpenApiConfig}.
     */
    public OpenApiConfig build() {
      return new OpenApiConfig(enabled, annotateFields, annotateGetters);
    }
  }
}
